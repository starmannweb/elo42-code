...
<div class="mgmt-header"><div><h1 class="mgmt-header__title"><?= $isEdit ? 'Editar benefício' : 'Novo benefício' ?></h1></div></div>
<div class="mgmt-form-card" style="max-width:640px;">
    <form method="POST" action="<?= $isEdit ? url('/admin/beneficios/' . $item['id'] . '/editar') : url('/admin/beneficios') ?>"><?= csrf_field() ?>
        ...
        <div class="mgmt-form-actions"><button type="submit" class="btn btn--primary"><?= $isEdit ? 'Salvar' : 'Criar' ?></button><a href="<?= url('/admin/beneficios') ?>" class="btn btn--secondary">Cancelar</a></div>
    </form>
</div>
