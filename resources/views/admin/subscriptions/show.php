...
<div class="mgmt-header"><div><h1 class="mgmt-header__title">Assinatura #<?= $sub['id'] ?></h1><p class="mgmt-header__subtitle"><span class="badge badge--<?= $sub['status'] ?>"><?= ucfirst(e($sub['status'])) ?></span></p></div></div>
    ...
    <div class="mgmt-detail__sidebar">
        <div class="mgmt-form-card"><h3 class="mgmt-info-card__title">Atualizar</h3>
            <form method="POST" action="<?= url('/admin/assinaturas/' . $sub['id']) ?>"><?= csrf_field() ?>
                <div class="form-group"><label class="form-label" style="font-size:var(--text-xs);">Status</label><select name="status" class="form-select"><?php foreach (['trial','active','past_due','cancelled','expired'] as $st): ?><option value="<?= $st ?>" <?= $sub['status']===$st?'selected':'' ?>><?= ucfirst($st) ?></option><?php endforeach; ?></select></div>
