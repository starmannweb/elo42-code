...
<div class="mgmt-form-card" style="max-width:640px;">
    <form method="POST" action="<?= url('/admin/organizacoes/' . $org['id'] . '/editar') ?>"><?= csrf_field() ?>
        ...
        <div class="form-group"><label class="form-label">Plano</label><select name="plan" class="form-select"><option value="basic" <?= ($org['plan']??'')==='basic'?'selected':'' ?>>Basic</option><option value="pro" <?= ($org['plan']??'')==='pro'?'selected':'' ?>>Pro</option><option value="enterprise" <?= ($org['plan']??'')==='enterprise'?'selected':'' ?>>Enterprise</option></select></div>
